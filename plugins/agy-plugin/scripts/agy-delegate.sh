#!/usr/bin/env bash
#
# agy-delegate.sh — headless delegation wrapper for the agy executor (subvibe marketplace).
...
# **agy** executor CLI and get clean text back on stdout.
# This plugin is single-driver — the executor is fixed to `agy` (no --driver /
# SUBVIBE_DRIVER switch).
#
# Architecture: CLI-agnostic core (this file) + this plugin's driver
# (scripts/drivers/agy.sh). Core owns arg parsing, tier resolution, digest
...
#
# Tiers map to agy-specific model names, remappable per tier. Defaults via
# env: SUBVIBE_DEFAULT_TIER, SUBVIBE_TIMEOUT, SUBVIBE_DEFAULT_MODEL (exact name).
# Per-tier remaps: AGY_TIER_*. Explicit --model/--tier win.
#
set -euo pipefail
...
DRIVER="agy"  # fixed for this plugin; no env override
TIER_EXPLICIT=0
MODEL=""
YOLO=0
SANDBOX=0
...
if [ -z "$MODEL" ]; then
  if [ "$TIER_EXPLICIT" -eq 1 ]; then
    ...
    case "$TIER" in
      low|medium|high) ;;
      *) echo "agy-delegate: invalid default tier '$TIER' (set SUBVIBE_DEFAULT_TIER to low|medium|high); using medium" >&2; TIER="medium" ;;
    esac
...
if [ "$WARN_CHARS" -gt 0 ] && [ "${#OUT}" -gt "$WARN_CHARS" ]; then
  echo "agy-delegate: note: output is ${#OUT} chars (> ${WARN_CHARS}) — that looks like a raw dump, not a digest. Don't ingest this into the conductor's context: re-run with --digest, or have the executor summarize it first. (env SUBVIBE_DIGEST_WARN_CHARS tunes this; 0 disables.)" >&2
fi
