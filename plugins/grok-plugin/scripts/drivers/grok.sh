...
#
# drivers/grok.sh — Grok Build CLI (`grok`, x.ai/cli) driver for grok-delegate.sh.
#
# Sourced (not executed) by scripts/grok-delegate.sh. See docs/drivers.md for
...
driver_build_args() {
  ...
  for d in "${ADD_DIRS[@]:-}"; do
    ...
    if [ -z "$first" ]; then first="$d"; else
      echo "grok-delegate: note: grok supports one working dir (--cwd); ignoring extra --dir '$d'" >&2
    fi
  done
  ...
  if [ "$YOLO" -eq 0 ] && [ "$PRINT_CMD" -ne 1 ]; then
    ...
    case "$PROMPT" in
      *implement*|*scaffold*|*migrate*|*refactor*|*"write the file"*|*"create the file"*|*"edit the file"*)
        echo "grok-delegate: note: this looks like a write task but --yolo is not set — headless grok has nobody to approve tool calls, so writes will stall or be denied. Add --yolo (and run on a branch) to actually write files." >&2 ;;
    esac
    shopt -u nocasematch
  fi
  return 0
...
driver_no_guard_warning() {
  echo "grok-delegate: WARNING — no GNU \`timeout\`/\`gtimeout\` on PATH." >&2
  echo "grok-delegate:   unauthenticated headless \`grok -p\` hangs instead of failing; without a" >&2
  echo "grok-delegate:   wall-clock guard this call may never return. Install coreutils \`timeout\`." >&2
  return 0
}

driver_hang_hint() {
  echo "grok-delegate:   grok hangs headless when not authenticated — run \`grok login\` (or set XAI_API_KEY) and retry." >&2
  return 0
}
