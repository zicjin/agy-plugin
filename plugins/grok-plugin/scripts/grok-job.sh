#!/usr/bin/env bash
#
# grok-job.sh — background-job layer over grok-delegate.sh.
...
#
# Usage:
#   grok-job.sh start  [grok-delegate options] "task"   # -> prints a JOB_ID, returns now
#   grok-job.sh list                                        # jobs started from this dir
#   grok-job.sh status <id>                                 # running | done(rc) | failed
#   grok-job.sh result <id>                                 # print stdout (+rc) when finished
#   grok-job.sh cancel <id>                                 # terminate a running job
#
# Jobs live under ${GROK_JOBS:-~/.grok-jobs}/<id>/ (out, err, rc, meta).
#
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
DELEGATE="${SUBVIBE_DELEGATE:-$HERE/grok-delegate.sh}"
REG="${SUBVIBE_JOBS:-$HOME/.subvibe-jobs}"

die() { echo "grok-job: $*" >&2; exit 1; }
...
# Human label for a delegate exit code (mirrors grok-delegate.sh structured codes).
rc_label() {
  case "$1" in
    0)  echo 'ok' ;;
    2)  echo 'CLI failed' ;;
